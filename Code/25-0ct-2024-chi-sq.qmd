...
The book presents this as their philosophy on analytical workflow: 1. Plot the data and look at expected relationships 2. Translate your hypothesis into a statistical model and that model into R code 3. Before interpreting the results, ASSESS the assumptions of your model. 4. AFTER you have assessed model assumptions, you can interpret the output of your statistical results by interpreting the test statistic and associated p-value. 5. Integrate the modeling results into your original figure (adding predicted for fitted lines/points to your graph)
...
```{r}
totals <- lady %>% group_by(Habitat, colour) %>% summarise(total.number = sum(number)) #Gives same structure as the matrix we drew on the board, just as a dataframe.
```
...
# X-squared is test statistics
# p-val is probability of getting the x-squared value
# The p-val gotten here is same as 1.39 x 10^-5 or 0.0000139
```

How do we interpret this? There is a very small chance that we are seeing more black bugs in industrial and more red in rural habitats by chance. So we can reject our null hypothesis of no difference and state that there is an association between color morph abundance and habitat type.
